...
@click.option(
    "--summary-file", "-s", default=None, help="The file to save the summary to."
)
@click.option(
    "--transcription-file",
    "-t",
    default=None,
    help="The file to save the transcription to.",
)
@click.option(
    "--audio-format",
    "-f",
    default=None,
    help="The format of the audio file. Defaults to mp3 when it cannot be infered.",
)
@click.option(
    "--language",
    "-l",
...
def cli(podcast_url_or_path, summary_file, transcription_file, audio_format, language):
    ...
    if not os.getenv("OPENAI_API_KEY"):
        raise Exception(
            "OPENAI_API_KEY is not set. Please add your API key to your environment variables."
        )
    ...
    if podcast_url_or_path.lower().endswith(".txt") and os.path.isfile(
        podcast_url_or_path
    ):
    ...
    click.secho(
        "\nDo you have any questions about the podcast? (Or type 'exit' to quit)",
        fg="blue",
    )
